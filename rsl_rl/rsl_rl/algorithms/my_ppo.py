...
from rsl_rl.modules import Encoder, ForceEstimator
from rsl_rl.modules.rnd import RandomNetworkDistillation
from rsl_rl.storage import My_RolloutStorage as RolloutStorage
from rsl_rl.utils import string_to_callable

# Add Encoder and ForceEstimator to learn with force information
class MY_PPO:
    ...
    def __init__(
        ...
        # Encoder and ForceEstimator components
        self.feature_dim = feature_dim
        self.encoder = Encoder(num_obs=raw_obs_dim, feature_dim=self.feature_dim).to(self.device)
        self.force_estimator = ForceEstimator(feature_dim=self.feature_dim, num_force=9).to(self.device)
        # Create optimizer
        self.optimizer = optim.Adam(list(self.policy.parameters()) + list(self.encoder.parameters()) + list(self.force_estimator.parameters()), lr=learning_rate)
    ...
    def init_storage(
        ...
        # create rollout storage
        num_forces = 9
        feature_shape = [self.feature_dim]
    ...
    def process_env_step(self, rewards, dones, infos):
        ...
        self.transition.real_forces = infos["real_forces"].clone()
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            # -- actor
            latent_feature_batch = self.encoder(obs_batch)
            ...
            mean_force_loss += force_loss.item()
            # -- RND loss
    ...
    def broadcast_parameters(self):
        ...
        model_params = [
            ...
            self.force_estimator.state_dict()
        ]
        if self.rnd:
        ...
        self.encoder.load_state_dict(model_params[1])
        self.force_estimator.load_state_dict(model_params[2])
        if self.rnd:
    ...
    def reduce_parameters(self):
        ...
        grads += [param.grad.view(-1) for param in self.encoder.parameters() if param.grad is not None]
        grads += [param.grad.view(-1) for param in self.force_estimator.parameters() if param.grad is not None]
        if self.rnd:
        ...
        all_params = chain(
            ...
            self.force_estimator.parameters()
        )
        if self.rnd:

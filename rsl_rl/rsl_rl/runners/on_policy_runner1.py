...
class OnPolicyRunner1:
    ...
    def save(self, path: str, infos=None):
        ...
        saved_dict = {
         ...
         # ========== 新增：保存 Encoder 和 ForceEstimator ==========
        saved_dict["encoder_state_dict"] = self.alg.encoder.state_dict()
        saved_dict["force_estimator_state_dict"] = self.alg.force_estimator.state_dict()
        # ========== 新增：保存 Encoder 和 ForceEstimator ==========
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        # ========== 新增：加载 Encoder 和 ForceEstimator ==========
        if "encoder_state_dict" in loaded_dict:
            self.alg.encoder.load_state_dict(loaded_dict["encoder_state_dict"])
        else:
            print("Warning: encoder_state_dict not found in checkpoint. Encoder weights not loaded.")
        if "force_estimator_state_dict" in loaded_dict:
            self.alg.force_estimator.load_state_dict(loaded_dict["force_estimator_state_dict"])
        else:
            print("Warning: force_estimator_state_dict not found in checkpoint. ForceEstimator weights not loaded.")
    ...
    def get_inference_policy(self, device=None):
        ...
        if self.cfg["empirical_normalization"]:
            ...
            policy = lambda x: self.alg.policy.act_inference(self.alg.encoder(self.obs_normalizer(x)))  # noqa: E731
        else:
            policy = lambda x: self.alg.policy.act_inference(self.alg.encoder(x))
        return policy
    ...
    def train_mode(self):
        ...
        self.alg.force_estimator.train()
        # -- RND
        if self.alg.rnd:
            self.alg.rnd.train()
        # -- Normalization

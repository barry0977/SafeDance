...
class MultiMotionLoader:
    def __init__(self, motion_files: list[str], body_indexes: Sequence[int], device: str = "cpu"):
        ...
        for motion_file in motion_files:
            ...
            jp = torch.tensor(data["joint_pos"], dtype=torch.float32, device=self.data_device)
            jv = torch.tensor(data["joint_vel"], dtype=torch.float32, device=self.data_device)
            bp = torch.tensor(data["body_pos_w"], dtype=torch.float32, device=self.data_device)
            bq = torch.tensor(data["body_quat_w"], dtype=torch.float32, device=self.data_device)
            blv = torch.tensor(data["body_lin_vel_w"], dtype=torch.float32, device=self.data_device)
            bav = torch.tensor(data["body_ang_vel_w"], dtype=torch.float32, device=self.data_device)
    ...
    def get_motion_data_batch(self, motion_idx: int, time_steps_start: torch.Tensor,
        ...
        return {
            "joint_pos":
                self.joint_pos_list[motion_idx].to(self.device)[time_steps_tensor],
            "joint_vel":
                self.joint_vel_list[motion_idx].to(self.device)[time_steps_tensor],
            "body_pos_w":
                self._body_pos_w_list[motion_idx].to(self.device)[time_steps_tensor][:, self._body_indexes],
            "body_quat_w":
                self._body_quat_w_list[motion_idx].to(self.device)[time_steps_tensor][:, self._body_indexes],
            "body_lin_vel_w":
                self._body_lin_vel_w_list[motion_idx].to(self.device)[time_steps_tensor][:, self._body_indexes],
            "body_ang_vel_w":
                self._body_ang_vel_w_list[motion_idx].to(self.device)[time_steps_tensor][:, self._body_indexes],
        }
...
class MotionCommand(CommandTerm):
    ...
    def command(self) -> torch.Tensor:
        return torch.cat([self.motion_joint_pos, self.motion_joint_vel], dim=1)
